...
import { ref } from 'vue'
import { Dialog, DialogPanel, DialogTitle, Menu, MenuButton, MenuItem, MenuItems, Switch, SwitchGroup, SwitchLabel, TransitionChild, TransitionRoot } from '@headlessui/vue'
  ...
  <div bg-red>
    <TransitionRoot :show="!open" class="cursor-pointer top-4 right-4 z-99 fixed" enter="transform transition ease-in-out duration-500 sm:duration-700" enter-from="translate-x-full" enter-to="translate-x-0" leave="transform transition ease-in-out duration-500 sm:duration-700" leave-from="translate-x-0" leave-to="translate-x-full">
      <div class="flex flex-col items-end">
        ...
        <SwitchGroup as="div" class="rounded-full flex bg-green-400 mt-4 p-2 items-center">
          <SwitchLabel as="span" class="text-sm ml-3">
            <span class="font-medium mr-2 text-gray-700">Eval</span>
          </SwitchLabel>
          <Switch v-model="isEval" class="border-transparent rounded-full cursor-pointer border-2 flex-shrink-0 h-6 transition-colors ease-in-out w-11 duration-200 relative inline-flex focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600" :class="[isEval ? 'bg-blue-600' : 'bg-gray-200']">
            <span aria-hidden="true" class="bg-white rounded-full h-5 shadow transform transition ease-in-out ring-0 w-5 duration-200 pointer-events-none inline-block" :class="[isEval ? 'translate-x-5' : 'translate-x-0']" />
          </Switch>
        </SwitchGroup>
      </div>
    </TransitionRoot>
